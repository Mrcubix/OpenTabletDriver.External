...
{
    private ObservableCollection<PropertySettingPair> _propertySettingPairs = [];
    
    [ObservableProperty]
    private SerializablePluginSettingsStore? _store;

    [ObservableProperty]
    private ObservableCollection<SerializableProperty> _properties = [];
    ...
    public ObservableCollection<PropertySettingPair> PropertySettingPairs
    {
    ...
    public void PropertiesChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        {
            PropertySettingPairs.Clear();
            PropertySettingPairs.AddRange(Properties.Select(p => new PropertySettingPair(p, Store[p])));
        }
    }
}